...
void main(){
    ...
    if (ValidatedType == "s"){
        ...
        List<int> Items = CheckForsalary();
        Console.WriteLine($"{simpleemployee.FullName}'s Salary is : {simpleemployee.CalculateSalary(float.Parse($"{Items[0]}.{Items[1]}"),Items[2],Items[3],Items[4]):C}");
    }
        ...
        List<int> Items = CheckForsalary();
        Console.WriteLine($"{serviceemployee.FullName}'s Salary is : {serviceemployee.CalculateSalary(float.Parse($"{Items[0]}.{Items[1]}"),Items[2],Items[3],Items[4]):C}");
    }
        ...
        List<int> Items = CheckForsalary();
        Console.WriteLine($"{manageremployee.FullName}'s Salary is :  {manageremployee.CalculateSalary(float.Parse($"{Items[0]}.{Items[1]}"),Items[2],Items[3],Items[4]):C}");
    }
        ...
        List<int> Items = CheckForsalary();
        Console.WriteLine($"{ceoemployee.FullName}'s Salary is : {ceoemployee.CalculateSalary(float.Parse($"{Items[0]}.{Items[1]}"),Items[2],Items[3],Items[4]):C}");
    }
}
...
List<int> CheckForsalary(){
    Console.WriteLine("Do you want to calculate the employee's salary ? ");
    string ValidatedKey = GetKey("yn");
    if (ValidatedKey == "n")Environment.Exit(0);
    Console.Clear();
    Console.WriteLine("Enter the number of months this employee has worked ");
    int NumberOfMonths = Convert.ToInt32(GetNumber());

    Console.WriteLine("Enter the floating point of the number of months this employee has worked ");
    int NumberOfMonthsFloatingPoint = Convert.ToInt32(GetNumber());

    Console.WriteLine("Enter the number of this employee's working hours ");
    int WorkingHours = Convert.ToInt32(GetNumber());

    Console.WriteLine("Enter the number of this employee's family salary ");
    int FamilySalary = Convert.ToInt32(GetNumber());

    Console.WriteLine("Enter the number of hours this employee has worked overtime ");
    int Overtime = Convert.ToInt32(GetNumber());
    Console.Clear();
    return [NumberOfMonths,NumberOfMonthsFloatingPoint,WorkingHours,FamilySalary,Overtime];
}
main();